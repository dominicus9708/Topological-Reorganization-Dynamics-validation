...
def estimate_best_lag(reference: np.ndarray, target: np.ndarray, max_lag: int) -> int:
    ...
    for lag in range(-max_lag, max_lag + 1):
        ...
        score = float(np.dot(ref, shifted))
        if score > best_score:
            best_score = score
            best_lag = lag
    return best_lag
...
def minmax_positive(arr: np.ndarray, eps: float = 1e-12) -> np.ndarray:
    amin = float(np.min(arr))
    amax = float(np.max(arr))
    if amax - amin <= eps:
        return np.full_like(arr, eps, dtype=float)
...
def build_topological_response(merged_df: pd.DataFrame, summary_input_df: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    min_event_half_window_s = max(32.0, duration_s / 128.0)
    ...
    # ----- Stage 3: local effective structural-dimension proxy layer -----
    sigma_reorg_event_vals = Sigma_reorg[event_mask]
    sigma_reorg_plus_vals = minmax_positive(sigma_reorg_event_vals)
    Sigma_reorg_plus = np.full(len(out), np.nan, dtype=float)
    Sigma_reorg_plus[event_mask] = sigma_reorg_plus_vals

    # Data-derived scale factors, not fixed arbitrary constants.
    # eta_em uses the in-window spread of Sigma_reorg_plus to place EM and detector proxies on comparable scales.
    eta_em = float(np.max(sigma_reorg_plus_vals) - np.min(sigma_reorg_plus_vals)) if len(sigma_reorg_plus_vals) else 1.0
    if eta_em <= 0:
        eta_em = 1.0

    # beta_reorg is chosen so the detector-side proxy uses the same normalized scale.
    beta_reorg = 1.0

    alpha_src_proxy = 3.0 + eta_em * P_em
    alpha_det_proxy = np.full(len(out), np.nan, dtype=float)
    alpha_det_proxy[event_mask] = 3.0 - beta_reorg * Sigma_reorg_plus[event_mask]

    sigma_alpha = np.full(len(out), np.nan, dtype=float)
    sigma_alpha[event_mask] = alpha_src_proxy[event_mask] - alpha_det_proxy[event_mask]

    # ----- Stage 4: response / relaxation layer -----
    ...
    valid_alpha = sigma_alpha[event_mask]
    valid_alpha = valid_alpha[~np.isnan(valid_alpha)]
    if len(valid_alpha) == 0:
        raise ValueError("sigma_alpha is empty inside the event window.")

    sigma_alpha_for_decay = sigma_alpha.copy()
    sigma_alpha_for_decay[event_mask] = sigma_alpha[event_mask] - np.nanmin(sigma_alpha[event_mask]) + 1e-12
    sigma_alpha_for_decay[~event_mask] = np.nan

    sigma_alpha_decay_series = np.where(np.isnan(sigma_alpha_for_decay), 0.0, sigma_alpha_for_decay)
    e_idx_alpha, tau_decay_alpha, decay_status_alpha = first_efold_time(t, sigma_alpha_decay_series, focus_idx)
    if tau_decay_alpha <= 0:
        raise ValueError("Could not derive positive tau_decay_alpha.")
    kappa_alpha = 1.0 / tau_decay_alpha

    sigma_alpha_peak = float(sigma_alpha[focus_idx])
    sigma_alpha_model = np.full(len(out), np.nan, dtype=float)
    post_mask = (t >= t_focus) & event_mask
    if not np.any(post_mask):
        post_mask[focus_idx:event_end_idx + 1] = True
    sigma_alpha_model[post_mask] = sigma_alpha_peak * np.exp(-kappa_alpha * (t[post_mask] - t_focus))

    delta_sigma_alpha = np.full(len(out), np.nan, dtype=float)
    delta_sigma_alpha[post_mask] = sigma_alpha[post_mask] - sigma_alpha_model[post_mask]
    ...
    out["Sigma_reorg_plus"] = Sigma_reorg_plus

    out["eta_em"] = eta_em
    out["beta_reorg"] = beta_reorg
    out["alpha_src_proxy"] = alpha_src_proxy
    out["alpha_det_proxy"] = alpha_det_proxy
    out["sigma_alpha_obs"] = sigma_alpha
    out["sigma_alpha_model_postfocus"] = sigma_alpha_model
    out["delta_sigma_alpha_postfocus"] = delta_sigma_alpha

    out["E_proxy"] = E_proxy
    out["sigma_topo_obs"] = sigma_topo
    out["propagation_proxy_event"] = propagation_proxy_event

    valid_sigma_alpha = sigma_alpha[event_mask]
    valid_sigma_alpha = valid_sigma_alpha[~np.isnan(valid_sigma_alpha)]
    valid_delta_alpha = delta_sigma_alpha[post_mask]
    valid_delta_alpha = valid_delta_alpha[~np.isnan(valid_delta_alpha)]
    ...
    summary = {
        ...
        "rms_v1_event": rms_v1,
        "eta_em": eta_em,
        "beta_reorg": beta_reorg,
        "tau_decay_alpha": tau_decay_alpha,
        "kappa_alpha": kappa_alpha,
        "decay_status_alpha": decay_status_alpha,
        ...
        "sigma_reorg_min": float(np.nanmin(Sigma_reorg[event_mask])),
        "sigma_reorg_max": float(np.nanmax(Sigma_reorg[event_mask])),
        "alpha_src_proxy_min": float(np.nanmin(alpha_src_proxy[event_mask])),
        "alpha_src_proxy_max": float(np.nanmax(alpha_src_proxy[event_mask])),
        "alpha_det_proxy_min": float(np.nanmin(alpha_det_proxy[event_mask])),
        "alpha_det_proxy_max": float(np.nanmax(alpha_det_proxy[event_mask])),
        "sigma_alpha_min": float(np.min(valid_sigma_alpha)) if len(valid_sigma_alpha) else np.nan,
        "sigma_alpha_max": float(np.max(valid_sigma_alpha)) if len(valid_sigma_alpha) else np.nan,
        ...
        "postfocus_rmse_sigma_alpha": float(np.sqrt(np.mean(valid_delta_alpha ** 2))) if len(valid_delta_alpha) else np.nan,
        "postfocus_mae_sigma_alpha": float(np.mean(np.abs(valid_delta_alpha))) if len(valid_delta_alpha) else np.nan,
    }
    return out, summary
...
def build_summary_df(summary: dict) -> pd.DataFrame:
    order = [
        ...
        ("eta_em", "Derived EM-side structural-dimension scale factor."),
        ("beta_reorg", "Derived detector-side structural-dimension scale factor."),
        ("tau_decay_alpha", "Derived structural-dimension-gap e-fold decay time."),
        ("kappa_alpha", "Derived relaxation rate 1/tau_decay_alpha."),
        ("decay_status_alpha", "Status of the structural-dimension-gap decay detection."),
        ...
        ("alpha_src_proxy_min", "Minimum source-side structural-dimension proxy."),
        ("alpha_src_proxy_max", "Maximum source-side structural-dimension proxy."),
        ("alpha_det_proxy_min", "Minimum detector-side structural-dimension proxy."),
        ("alpha_det_proxy_max", "Maximum detector-side structural-dimension proxy."),
        ("sigma_alpha_min", "Minimum structural-dimension-gap response inside the event window."),
        ("sigma_alpha_max", "Maximum structural-dimension-gap response inside the event window."),
        ...
        ("postfocus_rmse_sigma_alpha", "RMSE between observed and modeled structural-dimension-gap response after focus."),
        ("postfocus_mae_sigma_alpha", "MAE between observed and modeled structural-dimension-gap response after focus."),
    ]
...
def build_report(summary_df: pd.DataFrame, output_dir: Path) -> str:
    ...
    lines = []
    lines.append("GW170817 topological report (004)")
    lines.append("================================")
    lines.append("")
    lines.append("Purpose")
    lines.append("-------")
    lines.append("This version reinterprets the GW170817 response in terms of a local effective structural-dimension gap. The source-side proxy is constrained by the EM event proxy, while the detector-side proxy is constructed from the aligned common waveform proxy and its reconstruction residual.")
    lines.append("")
    ...
    lines.append("")
    lines.append("Reconstruction and structural-dimension layer")
    lines.append("----------------------------------------------")
    for key in [
        ...
        "alpha_src_proxy_min", "alpha_src_proxy_max",
        "alpha_det_proxy_min", "alpha_det_proxy_max",
        "sigma_alpha_min", "sigma_alpha_max",
    ]:
        if key in metric_map:
            lines.append(f"{key}: {metric_map[key]}")
    lines.append("")
    lines.append("Relaxation layer")
    lines.append("----------------")
    for key in [
        "tau_decay_alpha", "kappa_alpha", "decay_status_alpha",
        "propagation_proxy_event_max", "propagation_proxy_event_mean",
        "D_gw_mean", "D_gw_max",
        "postfocus_rmse_sigma_alpha", "postfocus_mae_sigma_alpha",
    ]:
        if key in metric_map:
            lines.append(f"{key}: {metric_map[key]}")
    lines.append("")
    ...
    lines.append("---------------------")
    lines.append("This topological stage should be read as a local effective structural-dimension model: the event window is selected first, then source-side and detector-side structural-dimension proxies are contrasted through sigma_alpha.")
    lines.append("")
...
def main() -> int:
    ...
    response_out = output_dir / "gw170817_topological_response_004.csv"
    summary_out = output_dir / "gw170817_topological_summary_004.csv"
    report_out = output_dir / "gw170817_topological_report_004.txt"
