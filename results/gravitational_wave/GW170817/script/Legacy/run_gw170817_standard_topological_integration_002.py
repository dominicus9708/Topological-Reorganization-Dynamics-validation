...
def add_smoothed_standard_columns(df: pd.DataFrame, sample_rate_hz: float, smooth_window_s: float) -> pd.DataFrame:
    out = df.copy()
    window_n = max(5, int(round(sample_rate_hz * smooth_window_s)))
    out["abs_mean_standard"] = np.abs(out["strain_mean_h1_l1_v1"].astype(float))
    out["abs_mean_standard_smooth"] = (
        out["abs_mean_standard"].rolling(window=window_n, center=True, min_periods=1).median()
    )
    return out
...
def main() -> int:
    ...
    standard_df = add_smoothed_standard_columns(standard_df, sample_rate_hz=sample_rate_hz, smooth_window_s=args.smooth_window_s)
    ...
    # Graph 1: event selection layer only
    fig = plt.figure(figsize=(10, 6))
    ax = fig.add_subplot(1, 1, 1)
    if not standard_df.empty:
        ax.plot(
            standard_df["time_centered_s"],
            normalize_series(standard_df["abs_mean_standard_smooth"]),
            label="Standard smoothed |mean strain|",
        )
    if not topo_df.empty:
        ax.plot(
            topo_df["time_centered_s"],
            normalize_series(topo_df["S_event"]),
            label="Topological S_event",
        )
    ax.axvline(float(topo_metrics["t_focus"]), linestyle="--", label="Topological focus")
    ax.set_xlabel("time_centered_s")
    ax.set_ylabel("normalized value")
    ax.set_title("GW170817 event-selection comparison")
    ax.legend()
    fig.tight_layout()
    fig.savefig(output_dir / "gw170817_event_selection_comparison.png", dpi=150)
    plt.close(fig)

    # Graph 2: topological reconstruction layer only
    fig = plt.figure(figsize=(10, 6))
    ax = fig.add_subplot(1, 1, 1)
    if not topo_df.empty:
    ...
    ax.set_xlabel("time_centered_s")
    ax.set_ylabel("normalized value")
    ax.set_title("GW170817 topological reconstruction / structural-gap")
    ax.legend()
    fig.tight_layout()
    fig.savefig(output_dir / "gw170817_reconstruction_comparison.png", dpi=150)
    plt.close(fig)

    # Graph 3: response layer only
    fig = plt.figure(figsize=(10, 6))
    ax = fig.add_subplot(1, 1, 1)
    if not topo_df.empty:
        ax.plot(topo_df["time_centered_s"], normalize_series(topo_df["sigma_alpha_obs"]), label="sigma_alpha_obs")
        ax.plot(topo_df["time_centered_s"], normalize_series(topo_df["sigma_alpha_model_postfocus"]), label="sigma_alpha_model_postfocus")
    ax.axvline(float(topo_metrics["t_focus"]), linestyle="--", label="Topological focus")
    ax.set_xlabel("time_centered_s")
    ax.set_ylabel("normalized value")
    ax.set_title("GW170817 structural-gap response vs model")
    ax.legend()
    fig.tight_layout()
    fig.savefig(output_dir / "gw170817_response_model_comparison.png", dpi=150)
    plt.close(fig)

    # Graph 4: time-only bar chart
    time_labels = [
    ...
    ax.set_title("GW170817 time-scale comparison")
    ax.tick_params(axis="x", rotation=20)
    fig.tight_layout()
    fig.savefig(output_dir / "gw170817_time_scale_bars.png", dpi=150)
    plt.close(fig)

    # Graph 5: error-only bar chart
    error_labels = ["topo_rmse", "topo_mae"]
    error_values = [
    ...
    ax.set_title("GW170817 topological error metrics")
    fig.tight_layout()
    ...
    report_lines = [
        "GW170817 standard-topological integration report (002)",
        "=====================================================",
        "",
        ...
        "gw170817_event_selection_comparison.png",
        "gw170817_reconstruction_comparison.png",
        "gw170817_response_model_comparison.png",
        "gw170817_time_scale_bars.png",
